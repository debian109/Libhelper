...
import UIHelper.MaterialIntro.animations.wrappers.BackButtonTranslationWrapper;
import UIHelper.MaterialIntro.animations.wrappers.NextButtonTranslationWrapper;
import UIHelper.MaterialIntro.animations.wrappers.PageIndicatorTranslationWrapper;
import UIHelper.MaterialIntro.animations.wrappers.SkipButtonTranslationWrapper;
import UIHelper.MaterialIntro.animations.wrappers.ViewPagerTranslationWrapper;
import UIHelper.MaterialIntro.listeners.IPageScrolledListener;
import UIHelper.MaterialIntro.listeners.IPageSelectedListener;
import UIHelper.MaterialIntro.listeners.MessageButtonBehaviourOnPageSelected;
import UIHelper.MaterialIntro.listeners.SwipeStateTouchListener;
import UIHelper.MaterialIntro.listeners.ViewBehavioursOnPageChangeListener;
import UIHelper.MaterialIntro.listeners.clickListeners.PermissionNotGrantedClickListener;
import UIHelper.MaterialIntro.listeners.scrollListeners.ParallaxScrollListener;
import UIHelper.MaterialIntro.widgets.InkPageIndicator;
import UIHelper.MaterialIntro.widgets.SwipeableViewPager;
import vn.namtran.basichelper.R;

import static android.view.View.GONE;

public abstract class BaseMaterialIntroActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.material_intro_activity);
    ...
    public void showPermissionsNotGrantedError() {
        showError(getString(R.string.material_intro_please_grant_permissions));
    }
    ...
    private void nextButtonBehaviour(final int position, final SlideFragment fragment) {
        boolean hasPermissionToGrant = fragment.hasNeededPermissionsToGrant();
        if (hasPermissionToGrant) {
            nextButton.setImageDrawable(ContextCompat.getDrawable(this, R.drawable.material_intro_ic_next));
            nextButton.setOnClickListener(permissionNotGrantedClickListener);
        } else if (adapter.isLastSlide(position)) {
            nextButton.setImageDrawable(ContextCompat.getDrawable(this, R.drawable.material_intro_ic_finish));
